...
In progress
- keybindings
- getting completion working again after poetry
...
This seems to get me in virtualenv?  
But it also breaks linting, because in the virtualenv the globally installed pylint doesn't seem to be available.  
I might be ok with just putting linters in all the environments?

It looks like the way to get linting going is just to install *pylint* in every environment.  I can get behind that.
Not sure if completion will work either, but installing *jedi* in every environment to see.
...
There's a suggestion in the [[https://elpy.readthedocs.io/en/latest/ide.html#syntax-checking][elpy docs]] that maybe I can get around the specific per-project installation: 

#+BEGIN_QUOTE
 It is possible to create a single virtual env for the sole purpose of installing flake8 in there, and then simply link the command script to a 
 directory inside your PATH, meaning you do not need to install the program in every virtual env separately.
 #+END_QUOTE

*Update*: following the strategy outlined [[https://stackoverflow.com/a/65740776/4386239][here]] I've created a virtualenv with 

flake8 jedi black autopep8 pylint yapf

and symlinked into PATH to avoid these virtualenv/linting problems.

But this still isn't complete, because jedi doesn't seem to have a single binary.  The jedi docs suggests that most installations are editor-specific anyway... 
so... is this already in company??  Doesn't seem to be.

I also happen to see that I haven't actually installed company-anaconda (oops) despite invoking it here.  and it doesn't seem to be necessary, per the company docs?

Ok, this is weird.  I'm going to try to get completions going and also see if flake8 works in the virtualenv, if so, then maybe I'll work on completions more.  

Yet another update: flake8 works in venv from commandline after being simlinked onto PATH, not before.  So I think we're solid except for completion investigation.

**Another update** 

completion automatically seems to work for definitions in current buffer.  For external completions, right now, anaconda-mode-complete throws 

Traceback (most recent call last):
  File "/Users/myuser/.emacs.d/anaconda-mode/0.1.13/service_factory-0.1.6-py3.8.egg/service_factory/providers/basehttp.py", line 40, in do_POST
    status, response = self.server.service(data)
  File "/Users/myuser/.emacs.d/anaconda-mode/0.1.13/service_factory-0.1.6-py3.8.egg/service_factory/service.py", line 50, in __call__
    result = self.apply(method, args)
  File "/Users/myuser/.emacs.d/anaconda-mode/0.1.13/service_factory-0.1.6-py3.8.egg/service_factory/service.py", line 105, in apply
    server_error(args['id'], error)
  File "/Users/myuser/.emacs.d/anaconda-mode/0.1.13/service_factory-0.1.6-py3.8.egg/service_factory/errors.py", line 91, in server_error
...
--- but this may be due to the backend confusion, so I might try putting anaconda-company in and seeing what happens.

looking at the \*anaconda-mode\* buffer, however, does seem to suggest that it automatically installs jedi for me.  There's a log of it doing so.  So that's good.
