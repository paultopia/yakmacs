...
; keybindings for this down below.
...
#+BEGIN_SRC emacs-lisp
  ...
  ; temporary until I get finished working on this file
...
** git
   ...
   - I guess I could use magit?  but it's more complex than git itself. 
     - keybindings I would need:
       - ~magit-stage-file~ for current file
       - ~magit-stage-modified~ is basically ~git add .~ I \think\?
       - ~magit-push-current-to-upstream~ appears to be ~git push~ although maybe ~magit-push-current-to-pushremote~ sometimes?
       - ~magit-commit-create~ to commit... I guess??
** terminal    
